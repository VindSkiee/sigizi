...
const statusConfig = {
  PENDING: { label: 'BARU', variant: 'info' as const, badgeClass: 'bg-blue-100 text-blue-700' },
  CONFIRMED: { label: 'DIPROSES', variant: 'warning' as const, badgeClass: 'bg-yellow-100 text-yellow-700' },
  DELIVERED: { label: 'SELESAI', variant: 'success' as const, badgeClass: 'bg-green-100 text-green-700' },
  CANCELLED: { label: 'GAGAL', variant: 'danger' as const, badgeClass: 'bg-red-100 text-red-700' },
};
...
const fallbackOrders: Order[] = [
  {
    ...
    items: [
      { id: '1', name: 'Beras Premium', quantity: 20, unitPrice: 11500, subtotal: 230000 },
      { id: '2', name: 'Ayam Potong', quantity: 5, unitPrice: 34000, subtotal: 170000 },
      { id: '3', name: 'Sayur Bayam', quantity: 25, unitPrice: 7500, subtotal: 187500 },
    ],
    total: 615000,
    status: 'PENDING',
...
export default function PesananPage() {
  ...
  const formatItems = (items: OrderItem[]) => {
    return items.map((item) => `${item.name} ${item.quantity}${item.unitPrice ? 'kg' : ''}`).join(', ');
  };
            ...
            <div key={i} className="bg-white rounded-xl border border-gray-200 p-5 shadow-sm animate-pulse">
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  ...
                  <div className="h-3 bg-gray-200 rounded w-60 mb-2"></div>
              ...
              <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4">
                <div className="flex-1">
                  <div className="flex items-center gap-3 mb-2">
                    ...
                    <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold ${statusConfig[order.status].badgeClass}`}>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden animate-in zoom-in-95 duration-200">
            <div className="flex justify-between items-center p-5 border-b border-gray-100 bg-gray-50">
              <h3 className="text-lg font-semibold text-gray-800">Detail Pesanan {selectedOrder.id}</h3>
            ...
            <div className="p-6 space-y-4">
              <div>
                ...
                <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold ${statusConfig[selectedOrder.status].badgeClass}`}>
              ...
              <div>
                ...
                {selectedOrder.items.map((item) => (
                  <div key={item.id} className="flex justify-between py-2 border-b border-gray-50">
                    ...
                    <span className="text-sm font-medium">Rp {item.subtotal.toLocaleString()}</span>
                  </div>
                ))}
              </div>
              <div className="flex justify-between pt-2 border-t">
                <span className="font-semibold">Total</span>
                <span className="font-bold text-lg">Rp {selectedOrder.total.toLocaleString()}</span>
              </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-3xl overflow-hidden animate-in zoom-in-95 duration-200">
            <div className="flex justify-between items-center p-5 border-b border-gray-100 bg-gray-50">
              ...
              <button onClick={() => setShowPaymentModal(null)} className="text-gray-400 hover:text-gray-600 transition-colors p-1 rounded-full hover:bg-gray-200">
                <X size={20} />
              </button>
            </div>
